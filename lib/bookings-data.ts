...
// No account/login system exists, so a customer manages their own booking
// via a real, unguessable per-booking token (128-bit UUID, generated by
// the column default on every insert) embedded in a link in their
// confirmation email — see app/(site)/manage-booking/[token]/page.tsx.
...
export async function getBookingByToken(token: string): Promise<BookingRow | null> {
  ...
  const { data, error } = await supabase
    ...
    .maybeSingle();
  if (error) {
    console.error("Failed to load booking by token:", error.message);
    return null;
  }
  return data as unknown as BookingRow | null;
}

// Blocks a repair already logged in completed_repairs — a customer
// shouldn't be able to cancel or reschedule something that already
// happened. See cancel_booking_by_token()'s ALREADY_COMPLETED check.
...
// A 'confirmed' booking re-checks capacity atomically (same guarantee as
// claimBookingSlot); a 'requested' booking (always the out-of-radius path)
// just updates the preference, no capacity check — see
// reschedule_booking_by_token()'s implementation for why the two differ.
export async function rescheduleBookingByToken(token: string, date: string, window: string) {
  const supabase = getSupabaseAdmin();
