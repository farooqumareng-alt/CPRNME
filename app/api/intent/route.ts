...
// Loose allow-lists mirroring the option ids in ProblemSelector.tsx, so a
// scripted POST can't stuff arbitrary strings into device/problem. Kept here
// (not imported from the client component) so this route has no dependency
// on a "use client" file.
...
export async function POST(request: Request) {
  ...
  const { device, problem, zip, sourcePage, sessionId } = body as Record<string, unknown>;
